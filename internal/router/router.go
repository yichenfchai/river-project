...
func Setup(r *gin.Engine, deps Dependencies) {
	...
	// 地理位置 — 无需鉴权
	r.GET("/api/v1/geo/canal-intro", deps.GeoHandler.CanalIntro)
	r.GET("/api/v1/geo/coords", deps.GeoHandler.CanalIntroByCoords)
	...
	// LLM 会话管理 — 需鉴权
	llmSessionGroup := r.Group("/api/v1/llm/chat")
	{
		llmSessionGroup.GET("/sessions", deps.LLMHandler.ListSessions)
		llmSessionGroup.GET("/sessions/:session_id", deps.LLMHandler.GetSessionMessages)
		llmSessionGroup.DELETE("/sessions/:session_id", deps.LLMHandler.DeleteSession)
	}
	...
	// 科普故事 — 无需鉴权，数据库持久化
	r.GET("/api/v1/stories", deps.StoryHandler.List)
	r.GET("/api/v1/stories/:id", deps.StoryHandler.GetByID)
	...
	adminUserGroup.Use(auth.RequireRole("admin"))
	{
		...
		adminUserGroup.POST("/users/:user_id/reset-password", deps.AdminHandler.ResetUserPassword)
	}
}
