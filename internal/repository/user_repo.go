...
func (r *userRepo) UpdatePassword(ctx context.Context, id, password string) error {
	res := r.db.WithContext(ctx).Model(&model.User{}).Where("id = ?", id).Update("password", password)
	if res.Error != nil {
		return apperrors.WrapDefault(apperrors.ErrDatabaseError, res.Error)
	}
	if res.RowsAffected == 0 {
...
func (r *userRepo) Seed(ctx context.Context) error {
	password111, err := bcrypt.GenerateFromPassword([]byte("111"), bcrypt.DefaultCost)
	if err != nil {
		return fmt.Errorf("bcrypt hashing failed: %w", err)
	}
	pass := string(password111)

	seeds := []model.User{
		{
			ID: UUID5("grand-canal-user", "111user"), Username: "111user",
			Password: pass, Email: "user111@test.dev", Nickname: "运河小舟",
			Role: "user", Status: "active",
		},
		{
			ID: UUID5("grand-canal-user", "111monitor"), Username: "111monitor",
			Password: pass, Email: "monitor111@test.dev", Nickname: "生态监测员",
			Role: "monitor", Status: "active",
		},
		{
			ID: UUID5("grand-canal-user", "111admin"), Username: "111admin",
			Password: pass, Email: "admin111@test.dev", Nickname: "运河管理员",
