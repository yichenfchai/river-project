#!/bin/bash
# deploy.sh — 一键部署
# 用法: DEPLOY_SERVER=ubuntu@1.2.3.4 bash deploy.sh [--frontend] [--backend] [--all]

set -e
SERVER="${DEPLOY_SERVER:?请设置环境变量 DEPLOY_SERVER，如: DEPLOY_SERVER=ubuntu@1.2.3.4}"
REMOTE_DIR="/opt/gcg"
...
echo ""
echo "部署完成 ✓"
echo "验证: curl http://${SERVER##*@}:8081/health"
