...
func main() {
	...
	if err := userRepo.Seed(context.Background()); err != nil {
		log.Printf("[WARN] 用户种子数据写入失败: %v", err)
	} else {
		log.Println("[DB] 测试用户种子数据就绪")
	}
	...
	if err := storyRepo.Seed(context.Background()); err != nil {
		log.Printf("[WARN] 故事种子数据写入失败: %v", err)
	} else {
		log.Println("[DB] 科普故事种子数据就绪")
	}
	storySvc := service.NewStoryService(storyRepo)
		storyHandler := handler.NewStoryHandler(storySvc)

	log.Println("[DB] 科普故事模块就绪")

	if err := postRepo.Seed(context.Background()); err != nil {
		log.Printf("[WARN] 帖子种子数据写入失败: %v", err)
	} else {
		log.Println("[DB] 帖子/评论种子数据就绪")
	}
	...
	geoSvc := service.NewGeoService(a.Logger)
	geoHandler := handler.NewGeoHandler(geoSvc)

	log.Println("[DB] 地理IP/运河城市模块就绪")
