...
async function submit() {
  ...
  try {
    ...
    msg.success(`答题完成！正确 ${res.data.correct_count}/${res.data.total_count}`)
  } catch {
    msg.error('提交失败')
  } finally {
...
@media (max-width: 768px) {
  ...
  .quiz-question h3 { font-size: 17px }
  .option-item { padding: 12px 14px; font-size: 14px }
  .quiz-action { flex-direction: column; align-items: stretch }
  .quiz-action .el-button { width: 100% }
...
@media (max-width: 480px) {
  .quiz-categories .el-button { font-size: 13px; padding: 8px 14px }
